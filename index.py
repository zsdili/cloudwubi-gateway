...
def load_phrase_dict():
    ...
    for fname in ("wubi86_recalc.txt", "wubi86_report.txt", "wubi86_gov.txt", "wubi86_long.txt", "wubi86_phrases.txt", "wubi86_daily.txt", "wubi86_classics.txt",
...
def context_associate(text, max_results=20):
    ...
    for n in range(min(16, len(text)), 0, -1):
        ...
        if key in SUCCESSION:
            ...
            for _n2 in (3, 2, 1):
                k2 = _strip_punct(text[-_n2:] if len(text) >= _n2 else text)
